/*
 * Map considerably harder bro.
 *
*/

var maphard = {
    load:{
        googleMap: function(mapId, mapOptions){
            mapId = typeof mapId === "string" ? document.getElementById(mapId) : mapId;
            var map = new google.maps.Map(mapId, mapOptions);
        },
        openLayersMap: function(mapId, mapOptions){
            var map = new OpenLayers.Map(mapId);
            
            mapOptions.layer === "default" ? map.addLayer(new OpenLayers.Layer.OSM()) : map.addLayer(new OpenLayers.Layer.OSM());

            var position = new OpenLayers.LonLat(-97.7428,30.2669).transform(new OpenLayers.Projection("EPSG:4326"), map.getProjectionObject());
            var zoom = 13;
